...
int ping(string target)
{
	...
	/*
	The hostent structure is used by functions to
	store information about a given host, such as 
	host name, IPv4 address, and so forth. 
	*/
	struct hostent *hp;
	/*
...
*/
	...
	while(cont)
	{
		...
		else if (retval)
		{
			...
			{
				//cout << "Recv: echo reply "<< target<<endl;
...
int main(int argc, char** argv)
{
	if (argc != 2)
	{
		cout << "Error (Input Missing): Please give Ip Address as Command line argument" << endl;
		exit(1);
	}
	if(ping(argv[1]))
		cout <<argv[1]<<endl;
